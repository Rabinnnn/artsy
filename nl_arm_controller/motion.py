...
JOINTS: tuple[str, ...] = ("_1", "_2", "_3", "_4", "_5", "_6")
...
def _normalize_joint(name: str) -> str:
    """Accept both '1' and '_1' — always return the '_N' form the SDK expects."""
...
class MotionExecutor:
    ...
    def _snap_to(self, pose: dict[str, float]) -> None:
        for joint, angle in pose.items():
            ...
            if not self.dry_run:
                self.robot.joints.set(j, angle, degrees=True)
            self._current_pose[j] = angle
